...
LaunchGameByNameCLI(gameName, deviceName, profilePath, games, gamesFile) {
    ; Get game data
    local exe, delay, launch, closeManager
    ...
    if (profilePath != "" && FileExist(profilePath)) {
        ; Simplified import for CLI
        ImportProfileCLI(profilePath, deviceName, gamesFile)
    }
...
; Wait for file dialog
    if !WinWait("ahk_class #32770", "", 5) {
        ; Don't show message in CLI mode, just return
        return
    }
    ...
    ; Wait for profile to load (use default delay since we don't have gameName here)
    Sleep(1000)  ; Default 1 second delay
...
ImportProfile(path, customDelay := "") {
    global selectedDevice, deviceGuiExe, gamesFile, defaultProfileDelay
...
LaunchGameByName(gameName, deviceName := "") {
    global games, gamesFile, defaultProfileDelay, selectedDevice, deviceGuiExe
...
selectDeviceProfileDirBtn := mainGui.AddButton("x40 y405 w380 h28", "Set Device Profile Folder")

settingsDevicesBox := mainGui.AddGroupBox("x20 y450 w460 h120", "Devices")

mainGui.AddText("x40 y480 w80 h20", "Device:")
devicesDrop := mainGui.AddDropDownList("x120 y478 w200 +Border")
addDeviceBtn := mainGui.AddButton("x330 y478 w70 h24", "Add")
deleteDeviceBtn := mainGui.AddButton("x405 y478 w70 h24", "Delete")

mainGui.AddText("x40 y510 w120 h20", "Profile Ext:")
profileExtDrop := mainGui.AddDropDownList("x120 y508 w140 +Border")
addExtEdit := mainGui.AddEdit("x270 y508 w70 h22 +Border")
addExtBtn := mainGui.AddButton("x345 y508 w80 h24", "Add Ext")

settingsExitBtn := mainGui.AddButton("x295 y580 w185 h30", "Exit Program")
